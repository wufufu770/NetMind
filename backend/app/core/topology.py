...
class TopologyGraph:
    def __init__(self):
        self.nodes=[
            ...
            {'id':'guest_terminal','type':'host','status':'limited','subnet':'192.168.10.0/24'},
            {'id':'s1','type':'switch','status':'healthy'},
            {'id':'meeting_server','type':'server','status':'healthy'},
            {'id':'lab_server','type':'server','status':'healthy','ip':'10.0.0.2'},
    ...
    def node_attr(self, node_id, key, default=None):
        n=next((x for x in self.nodes if x['id']==node_id), {})
        return n.get(key, default)

    def switches(self):
        return [n['id'] for n in self.nodes if n.get('type')=='switch']
